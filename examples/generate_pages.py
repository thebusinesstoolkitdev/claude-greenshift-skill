...
VAR_SURFACE = 'var(--gt-surface, #fbf6ec)'
...
def cta_purple(seed, h2txt, para, buttons):
    ...
    inner += block(seed + 'p', 'p', text=para, style={"color": [VAR_TEXT_MUTED], "textAlign": ["center"], "marginBottom": ["1.8rem"]})
...
who_text = block(s + 'wt', 'div', inner=
    ...
    block(s + 'wp1', 'p', text='At Marigold & Fern, we believe getting dressed should feel easy and fun. We curate high-quality, versatile pieces that move with the rhythm of your life — so you always feel polished, comfy and put-together.', style={"color": [VAR_TEXT_MUTED], "marginBottom": ["1rem"]}) +
    block(s + 'wp2', 'p', text="More than a shop, it's a friendly little spot to browse, get a second opinion, and leave with something you love.", style={"color": [VAR_TEXT_MUTED], "marginBottom": ["2rem"]}) +
...
def mv_card(seed, icon, label, labelcolor, h3txt, para):
    return block(seed, 'div', inner=
        ...
        block(seed + 'p', 'p', text=para, style={"color": [VAR_TEXT_MUTED], "fontSize": ["15px"], "marginBottom": ["0px"]}),
...
mission = section(s + 'mv',
    content(s + 'mvc',
        ...
        block(s + 'mvgrid', 'div', inner=
            mv_card(s + 'mc1', 'gt-icon-pansy-purple.png', 'Our Mission', 'var(--gt-primary, #7e5aa6)', 'Make dressing well easy', 'To take the guesswork out of getting dressed — with versatile, quality pieces that help every woman feel confident, comfy and put-together.') +
    ...
    bg=VAR_SURFACE_ALT, name='Mission Vision')
...
def love_card(seed, icon, h3txt, para):
    return block(seed, 'div', inner=
        ...
        block(seed + 'p', 'p', text=para, style={"color": [VAR_TEXT_MUTED], "fontSize": ["14px"], "textAlign": ["center"], "marginBottom": ["0px"]}),
        gclass='gt-card-surface-alt', name=h3txt)
...
love = section(s + 'love',
    ...
    bg=VAR_SURFACE, name='What Youll Love')
...
pages['about'] = block(s + 'page', 'div', inner=hero + who + mission + love + cta_a,
                       style={"marginBlockStart": ["0px"], "backgroundColor": [VAR_SURFACE]}, name='Page Wrapper', alignfull=True)
...
for fn, nm, cat, w, h in prods:
    ...
    cards += block(seed, 'article', html_attrs=f'data-cat="{catmap[cat]}"',
        ...
        block(seed + 'c', 'div', text=cat, style={"fontSize": ["12px"], "letterSpacing": ["0.7px"], "textTransform": ["uppercase"], "color": [VAR_TEXT_MUTED], "textAlign": ["center"]}),
        style={"display": ["flex"], "flexDirection": ["column"], "alignItems": ["center"]}, name=nm)

empty_state = rawhtml('<p id="gt-filter-empty" hidden style="text-align:center;color:var(--gt-text-muted,#6e6456);padding:2rem 0">Nothing in this category yet — check back soon.</p>')
...
def info_row(seed, icon, label, lines):
    ...
    return block(seed, 'div', inner=
        ...
        block(seed + 'col', 'div', inner=
            block(seed + 'l', 'div', text=label, style={"fontSize": ["12px"], "fontWeight": ["600"], "letterSpacing": ["1.2px"], "textTransform": ["uppercase"], "color": ["var(--gt-primary, #7e5aa6)"], "marginBottom": ["0.2rem"]}) + txt,
...
left = block(s + 'left', 'div', inner=
    ...
    block(s + 'lp', 'p', text="Prefer in person? Come browse and say hi. We're always happy to help you find something you love.", style={"color": [VAR_TEXT_MUTED], "marginBottom": ["1.8rem"]}) +
...
right = block(s + 'right', 'div', inner=
    ...
    block(s + 'rp', 'p', text="We'll get back to you within a day or two.", style={"color": [VAR_TEXT_MUTED], "fontSize": ["14px"], "marginBottom": ["1.6rem"]}) + form_html,
    gclass='gt-form-card', style={"backgroundColor": [VAR_SURFACE_ALT], "borderRadius": ["16px"], "paddingTop": ["2.4rem"], "paddingBottom": ["2.4rem"], "paddingLeft": ["clamp(1.2rem, 3vw, 2.2rem)"], "paddingRight": ["clamp(1.2rem, 3vw, 2.2rem)"], "boxSizing": ["border-box"]}, name='Form Card')
